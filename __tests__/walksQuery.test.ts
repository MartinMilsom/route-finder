...
import { query as WalksQuery } from "../queries/WalksQuery";
import {
  createTestClient,
...
describe("walks query", () => {
  it("returns data within a map area", async () => {
    ...
    var findQuery: jest.Mock<any, any> = jest.fn();
    var server = await testServer(data, findQuery);
    ...
    // when
    var result = await server.query(WalksQuery(latitude, longitude, radius));
...
async function testServer(data: any, find: jest.Mock<any, any>) {
