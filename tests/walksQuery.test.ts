...
describe("walks query", () => {
    it("returns filtered walk data", async () => {
        ...
        expect(result.data.walks[0]).toEqual({
            ...
            distance: {
                mile: 2.6382709782464304, 
                kilometer: 4.245893715896214
            }
        });  
    ...
    it("returns data less than distance", async () => {
        ...
        // when
        const result = await server.query(mapQuery(new WalksQuery(null, new DistanceQuery(null, max), null)));
    ...
    it("returns circular walk", async () => {
        // given
        const data = [ expectedDocument ];
        ...
        // when
        const result = await server.query(mapQuery(new WalksQuery(null, null, Direction.Circular)));
